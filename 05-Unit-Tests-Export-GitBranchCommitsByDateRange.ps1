...
foreach ($sectionScript in @(Get-ChildItem -LiteralPath (Join-Path -Path $PSScriptRoot -ChildPath '.ps-UnitTests\05-Unit-Tests-Export-GitBranchCommitsByDateRange') -Filter '*.ps1')) {
...
try {
    ...
    Import-PSGitRepoCommandsForDateRangeLookup -modulePath $resolvedModulePath
    $resolvedRepoPath = New-DatedTestFolderUnderTests -parentPath $repoPath
    $dateWindow = Assert-CommitsOnEighteenthOfFebruary2026 -libraryPath $libraryPath
    $byAuthor = Assert-DateRangeFilteredByAuthorAndEmail -libraryPath $libraryPath -rangeFrom $dateWindow.RangeFrom -rangeTo $dateWindow.RangeTo -dateTip $dateWindow.DateTip
    Export-CommitsForEighteenthOfFebruaryByAuthor -libraryPath $libraryPath -exportFolder $resolvedRepoPath -rangeFrom $dateWindow.RangeFrom -rangeTo $dateWindow.RangeTo -dateTip $dateWindow.DateTip -authorCommitCount $byAuthor.CommitCount
